...
"""
ACCURACY-A3.4 - Rho + Current (J) runner (opt-in).
...
class AccuracyA34Error(ValueError):
    pass


DEFAULT_INPUT_CSV = Path("data/accuracy/isomer_truth.v1.csv")
DEFAULT_OUT_DIR = Path("out_accuracy_a1_isomers_a3_4")
...
def _require_columns(fieldnames: list[str] | None, required: Iterable[str]) -> None:
    if not fieldnames:
        raise AccuracyA34Error("missing CSV header")
    missing = [c for c in required if c not in fieldnames]
    if missing:
        raise AccuracyA34Error(f"missing required columns: {missing}")
    ...
    for cyc in cycles_full:
        ...
        for a in cyc:
            if int(a) not in heavy_mapping:
                raise AccuracyA34Error("unexpected non-heavy atom in SSSR cycle")
...
def _build_weight_adjacency(
    ...
    *,
    ...
    if mode not in {"bond_order", "bond_order_delta_chi"}:
        raise AccuracyA34Error(f"invalid edge_weight_mode: {mode}")
    ...
    if mode == "bond_order_delta_chi":
        ...
        if missing:
            raise AccuracyA34Error(f"missing atoms_db chi for Z={missing}")
...
def _build_base_potential(
    ...
    *,
    ...
    if missing_eps:
        raise AccuracyA34Error(f"missing atoms_db epsilon for Z={missing_eps}")
    epsilon = np.asarray([float(eps_by_z[int(z)]) for z in row.types], dtype=float)

    variant = str(potential_variant)
    if variant != "epsilon_z":
        raise AccuracyA34Error(f"invalid potential_variant: {variant}")
...
def _mass_vector(types: Sequence[int], *, mode: str) -> np.ndarray:
    ...
    if mz == "sqrtZ":
        return np.sqrt(z)
    raise AccuracyA34Error(f"invalid mass_mode: {mz}")
...
def _heat_kernel_matrix_shifted_trace_normalized_hermitian(
    ...
    if tau_val <= 0.0 or not math.isfinite(tau_val):
        raise AccuracyA34Error("heat_tau must be > 0 and finite")

    H0 = np.asarray(H)
    if H0.ndim != 2 or H0.shape[0] != H0.shape[1] or H0.shape[0] == 0:
        raise AccuracyA34Error("H must be non-empty square")

    eigvals, eigvecs = np.linalg.eigh(H0)
    eigvals = np.asarray(eigvals).reshape(-1)
    eigvecs = np.asarray(eigvecs)
    if eigvals.size == 0:
        raise AccuracyA34Error("H must be non-empty")
    ...
    if not math.isfinite(trace_heat) or trace_heat <= 0.0:
        raise AccuracyA34Error("invalid trace_heat in rho-law")
...
def _rho_realness_and_normalize(
    ...
    *,
    ...
    if not math.isfinite(rho_imag_max) or rho_imag_max >= float(rho_imag_tol):
        raise AccuracyA34Error(f"rho_complex_violation: rho_imag_max={rho_imag_max}")

    rho_r = np.asarray(np.real(rho0), dtype=float).reshape(-1)
    if rho_r.size == 0:
        raise AccuracyA34Error("rho must be non-empty")

    rho_floor_val = float(rho_floor)
    if rho_floor_val < 0.0 or not math.isfinite(rho_floor_val):
        raise AccuracyA34Error("rho_floor must be >= 0 and finite")
    ...
    if not math.isfinite(rho_sum) or rho_sum <= 0.0:
        raise AccuracyA34Error("invalid rho_sum in rho normalization")
...
def _phi_from_rho(*, rho: np.ndarray, phi_eps: float, gauge_fix_phi_mean: bool) -> np.ndarray:
    ...
    if eps <= 0.0 or not math.isfinite(eps):
        raise AccuracyA34Error("phi_eps must be > 0 and finite")
...
def _j_from_heat_kernel_edges(
    *,
    ...
    """
    J definition (contract):
      q_ij := exp(-i * theta_ij) * K_ij  (gauge-invariant)
      j_edge(i,j) := abs(Im(q_ij))
    ...
    if K0.ndim != 2 or K0.shape[0] != K0.shape[1]:
        raise AccuracyA34Error("K must be square")
    if A0.shape != K0.shape:
        raise AccuracyA34Error("A must match K shape")
    n = int(K0.shape[0])

    j_node = np.zeros((n,), dtype=float)
    for i, j, *_rest in bonds:
        ...
        if a == b:
            continue
        theta = float(A0[a, b])
        q = np.exp(-1j * theta) * K0[a, b]
        j_edge = float(abs(np.imag(q)))
        if not math.isfinite(j_edge):
            raise AccuracyA34Error("non-finite j_edge")
        j_node[a] += j_edge
        j_node[b] += j_edge

    j_sum = float(np.sum(j_node))
    return np.asarray(j_node, dtype=float), float(j_sum)
...
def _mix_rho_and_j(
    *,
    ...
    Returns:
      (rho_eff, rho_eff_renorm_applied, rho_eff_sum, j_sum, j_entropy)
    """
    k = float(kappa)
    if k < 0.0 or k > 1.0 or not math.isfinite(k):
        raise AccuracyA34Error("kappa must be in [0,1] and finite")

    rho0 = np.asarray(rho, dtype=float).reshape(-1)
    j0 = np.asarray(j_node, dtype=float).reshape(-1)
    if rho0.size == 0 or j0.size != rho0.size:
        raise AccuracyA34Error("rho and j_node must have same non-zero length")

    j_sum = float(np.sum(j0))
    if j_sum > 0.0:
        j_norm = np.asarray(j0 / float(j_sum), dtype=float)
    else:
        j_norm = np.zeros_like(rho0)

    rho_eff = (1.0 - k) * rho0 + k * j_norm
    rho_eff_sum = float(np.sum(rho_eff))
    if not math.isfinite(rho_eff_sum) or rho_eff_sum <= 0.0:
        raise AccuracyA34Error("invalid rho_eff_sum")
    rho_sum_tol = 1e-8
    renorm_applied = bool(abs(rho_eff_sum - 1.0) > rho_sum_tol)
    if renorm_applied:
        rho_eff = np.asarray(rho_eff / float(rho_eff_sum), dtype=float)

    j_entropy = _entropy(j_norm) if j_sum > 0.0 else 0.0
    return np.asarray(rho_eff, dtype=float), bool(renorm_applied), float(rho_eff_sum), float(j_sum), float(j_entropy)
...
def _load_rows(input_csv: Path) -> list[_Row]:
    ...
    if not raw_rows:
        raise AccuracyA34Error("input_csv has no data rows")
    ...
    for r in raw_rows:
        ...
        if not mid or not gid or not smiles:
            raise AccuracyA34Error(f"invalid row (id/group_id/smiles required): {r}")
...
def _get_row_static(row: _Row, *, atoms_db: AtomsDbV1, cfg: _A34Config) -> dict[str, object]:
    key = str(row.mid)
    ...
    j_node, j_sum = _j_from_heat_kernel_edges(K=K, A=A, bonds=row.bonds)
    if float(j_sum) > 0.0:
        j_entropy = _entropy(np.asarray(j_node, dtype=float) / float(j_sum))
    else:
        j_entropy = 0.0
    ...
    cached = {
        ...
        "j_node": np.asarray(j_node, dtype=float),
        "j_sum": float(j_sum),
        "j_entropy": float(j_entropy),
...
def _solve_scf_full_functional_v1_rho_plus_current(
    *,
...
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, bool, float, float, float, list[dict[str, object]]]:
    lap = np.asarray(laplacian_base, dtype=float)
    if lap.ndim != 2 or lap.shape[0] != lap.shape[1]:
        raise AccuracyA34Error("laplacian_base must be square")
    n = int(lap.shape[0])

    v0_vec = np.asarray(v0, dtype=float).reshape(-1)
    if v0_vec.size != n:
        raise AccuracyA34Error("v0 must have shape (n,)")

    iters = int(sc_iters)
    if iters < 0 or iters > 5:
        raise AccuracyA34Error("sc_iters must be in [0,5]")
    ...
    if not math.isfinite(eta0):
        raise AccuracyA34Error("sc_eta must be finite")
    if damping < 0.0 or damping > 1.0 or not math.isfinite(damping):
        raise AccuracyA34Error("sc_damping must be in [0,1] and finite")
    if clip <= 0.0 or not math.isfinite(clip):
        raise AccuracyA34Error("sc_clip must be > 0 and finite")
    if max_bt < 0 or max_bt > 3:
        raise AccuracyA34Error("sc_max_backtracks must be in [0,3]")

    m0 = np.asarray(mvec, dtype=float).reshape(-1)
    if m0.size != n:
        raise AccuracyA34Error("mvec must have shape (n,)")
    ...
    if not (math.isfinite(alpha) and math.isfinite(beta) and math.isfinite(gamma)):
        raise AccuracyA34Error("coef_alpha/beta/gamma must be finite")

    rho0 = np.asarray(rho_base, dtype=float).reshape(-1)
    j0 = np.asarray(j_node, dtype=float).reshape(-1)
    if rho0.size != n or j0.size != n:
        raise AccuracyA34Error("rho_base/j_node must have shape (n,)")

    def _state() -> tuple[np.ndarray, np.ndarray, np.ndarray, float, bool, float, float, float, float]:
        rho_eff, rho_eff_renorm_applied, rho_eff_sum, j_sum, j_entropy = _mix_rho_and_j(
            rho=rho0, j_node=j0, kappa=float(kappa)
        )
        ...
        if not math.isfinite(E):
            raise AccuracyA34Error("non-finite E in SCF")
        return (
    ...
    v = v0_vec.copy()
    rho_eff, phi, curvature, E, rho_eff_renorm_applied, rho_eff_sum, j_sum, j_entropy, rho_eff_entropy = _state()
    ...
    for t in range(iters):
        ...
        for bt in range(max_bt + 1):
            ...
            rho_eff_c, phi_c, curvature_c, E_c, ren_c, sum_c, j_sum_c, j_ent_c, rho_eff_ent_c = _state()
...
def _predict_one(
    ...
    *,
    ...
    j_node = np.asarray(st["j_node"], dtype=float)
    ...
    (
        ...
        rho_eff_entropy,
        trace,
    ) = _solve_scf_full_functional_v1_rho_plus_current(
...
def run_a3_4(*, input_csv: Path, out_dir: Path, seed: int, experiment_id: str) -> None:
    cfg = _A34Config()
    ...
    if len(group_ids) < 2:
        raise AccuracyA34Error("need at least 2 groups for LOOCV")
    ...
    for fold_id, test_gid in enumerate(fold_order, start=1):
        ...
        if not test_rows:
            raise AccuracyA34Error("empty test group in LOOCV")
        ...
        if gm_test is None:
            raise AccuracyA34Error("missing group metrics for test group")
        fold_rows.append(
            {
                "fold_id": int(fold_id),
                "test_group_id": str(test_gid),
    ...
    j_sum_max = float(max(float(r.get("j_sum") or 0.0) for r in all_diag_records) if all_diag_records else 0.0)
...
def _write_outputs_a3_4(
    *,
    ...
    current_summary_path = out_dir / "current_summary.csv"
    with current_summary_path.open("w", encoding="utf-8", newline="") as f:
        fieldnames = [
            "fold_id",
            "id",
            "group_id",
    ...
    metrics_payload: dict[str, object] = {
        "schema_version": "accuracy_a1_isomers_a3_4.v1",
        ...
        "files": {
            ...
            "current_summary_csv": "current_summary.csv",
    ...
    index_lines = [
        f"# {experiment_id} (Isomers) A3.4 rho+current (J)",
        "",
    ...
    extra_files: list[tuple[Path, Path]] = [
        ...
        (Path("docs/specs/accuracy_a3_4_rho_plus_current.md"), repo_root / "docs/specs/accuracy_a3_4_rho_plus_current.md"),
    ]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="ACCURACY-A3.4 rho+current (J) runner (opt-in).")
    p.add_argument("--experiment_id", type=str, default="ACCURACY-A3.4", help="Experiment identifier for outputs.")
