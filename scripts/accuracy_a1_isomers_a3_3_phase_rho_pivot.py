...
"""
ACCURACY-A3.3 — Phase → Rho Pivot runner (opt-in).

Contract SoT:
  docs/specs/accuracy_a3_3_phase_rho_pivot.md

Fixators (must hold):
  1) Phase enters ONLY via rho (L_A only used to compute rho)
  2) rho realness + normalization sensors (rho_imag_max, rho_sum)
  3) Nested Phi selection per outer LOOCV fold (train-only)

This script is intentionally separate from A2 runners (zero-leak).
"""
...
class AccuracyA33Error(ValueError):
    pass


DEFAULT_INPUT_CSV = Path("data/accuracy/isomer_truth.v1.csv")
DEFAULT_OUT_DIR = Path("out_accuracy_a1_isomers_a3_3")
...
def _require_columns(fieldnames: list[str] | None, required: Iterable[str]) -> None:
    if not fieldnames:
        raise AccuracyA33Error("missing CSV header")
    missing = [c for c in required if c not in fieldnames]
    if missing:
        raise AccuracyA33Error(f"missing required columns: {missing}")
...
def _weighted_adjacency_from_bonds(
    *,
    ...
    if mode not in {"bond_order", "bond_order_delta_chi"}:
        raise AccuracyA33Error(f"invalid edge_weight_mode: {mode}")
    ...
    if mode == "bond_order_delta_chi":
        ...
        if missing:
            raise AccuracyA33Error(f"missing atoms_db chi for Z={missing}")
    ...
    for cyc in cycles_full:
        ...
        for a in cyc:
            if int(a) not in heavy_mapping:
                raise AccuracyA33Error("unexpected non-heavy atom in SSSR cycle")
...
def _load_rows(input_csv: Path) -> list[_Row]:
    ...
    if not raw_rows:
        raise AccuracyA33Error("input_csv has no data rows")
    ...
    for r in raw_rows:
        ...
        if not mid or not gid or not smiles:
            raise AccuracyA33Error(f"invalid row (id/group_id/smiles required): {r}")
...
def _build_base_potential(
    ...
    *,
    ...
    if missing_eps:
        raise AccuracyA33Error(f"missing atoms_db epsilon for Z={missing_eps}")
    epsilon = np.asarray([float(eps_by_z[int(z)]) for z in row.types], dtype=float)

    variant = str(potential_variant)
    if variant != "epsilon_z":
        raise AccuracyA33Error(f"invalid potential_variant: {variant}")
...
def _mass_vector(types: Sequence[int], *, mode: str) -> np.ndarray:
    ...
    if mz == "sqrtZ":
        return np.sqrt(z)
    raise AccuracyA33Error(f"invalid mass_mode: {mz}")


def _heat_kernel_diagonal_shifted_trace_normalized_hermitian(
    H: np.ndarray, *, tau: float
) -> tuple[np.ndarray, float, float, float, float, float]:
    """
    Hermitian heat kernel diagonal with stable spectral shift:
      rho = diag(exp(-tau * (H - lambda_min I))) / trace(exp(-tau * (H - lambda_min I))).

    Returns:
      (rho_raw_complex, trace_heat, lambda_min, lambda_max, rho_trace_norm, rho_imag_max)
    """
    tau_val = float(tau)
    if tau_val <= 0.0 or not math.isfinite(tau_val):
        raise AccuracyA33Error("heat_tau must be > 0 and finite")

    H0 = np.asarray(H)
    if H0.ndim != 2 or H0.shape[0] != H0.shape[1]:
        raise AccuracyA33Error("H must be square")
    if H0.shape[0] == 0:
        raise AccuracyA33Error("H must be non-empty")
    ...
    if not math.isfinite(trace_heat) or trace_heat <= 0.0:
        raise AccuracyA33Error("invalid trace_heat in rho-law")

    diag_heat = (np.conj(eigvecs) * eigvecs) @ weights
    rho_raw = np.asarray(diag_heat, dtype=np.complex128).reshape(-1) / float(trace_heat)
    rho_trace_norm = float(np.real(np.sum(rho_raw)))
    rho_imag_max = float(np.max(np.abs(np.imag(rho_raw)))) if rho_raw.size else 0.0
    return (
        np.asarray(rho_raw, dtype=np.complex128),
        float(trace_heat),
        float(lambda_min),
        float(lambda_max),
...
def _rho_realness_and_normalize(
    ...
    *,
    ...
    Returns:
      (rho_real_normed, rho_sum, rho_imag_max, rho_renorm_applied, rho_renorm_delta)
    """
    ...
    if not math.isfinite(rho_imag_max) or rho_imag_max >= float(rho_imag_tol):
        raise AccuracyA33Error(f"rho_complex_violation: rho_imag_max={rho_imag_max}")

    rho_r = np.asarray(np.real(rho0), dtype=float).reshape(-1)
    if rho_r.size == 0:
        raise AccuracyA33Error("rho must be non-empty")

    if float(rho_floor) > 0.0:
        rho_r = np.maximum(rho_r, float(rho_floor))

    rho_sum = float(np.sum(rho_r))
    rho_renorm_delta = float(rho_sum - 1.0)
    if not math.isfinite(rho_sum) or rho_sum <= 0.0:
        raise AccuracyA33Error("invalid rho_sum in rho normalization")

    rho_sum_tol = 1e-8
    rho_renorm_applied = bool(abs(rho_renorm_delta) > rho_sum_tol)
    rho_n = np.asarray(rho_r / float(rho_sum), dtype=float)
    return rho_n, float(rho_sum), float(rho_imag_max), bool(rho_renorm_applied), float(rho_renorm_delta)


def _phi_from_rho(*, rho: np.ndarray, phi_eps: float, gauge_fix_phi_mean: bool) -> np.ndarray:
    eps = float(phi_eps)
    if eps <= 0.0 or not math.isfinite(eps):
        raise AccuracyA33Error("phi_eps must be > 0 and finite")
...
def _solve_scf_full_functional_v1_phase_rho_pivot(
    *,
    laplacian_base: np.ndarray,
    laplacian_phase: np.ndarray,
    v0: np.ndarray,
    heat_tau: float,
    phi_eps: float,
    rho_floor: float,
    ...
    """
    A3.3 Fixator #1:
      - L_base used for E/curvature/SCF monotonicity
      - L_phase used ONLY to compute rho (via H_phase = L_phase + diag(V))
    """
    lap = np.asarray(laplacian_base, dtype=float)
    if lap.ndim != 2 or lap.shape[0] != lap.shape[1]:
        raise AccuracyA33Error("laplacian_base must be square")

    lap_p = np.asarray(laplacian_phase, dtype=np.complex128)
    if lap_p.ndim != 2 or lap_p.shape[0] != lap_p.shape[1]:
        raise AccuracyA33Error("laplacian_phase must be square")

    n = int(lap.shape[0])
    if lap_p.shape[0] != n:
        raise AccuracyA33Error("laplacian_base and laplacian_phase must have same shape")

    v0_vec = np.asarray(v0, dtype=float).reshape(-1)
    if v0_vec.size != n:
        raise AccuracyA33Error("v0 must have shape (n,)")

    iters = int(sc_iters)
    if iters < 0 or iters > 5:
        raise AccuracyA33Error("sc_iters must be in [0,5]")

    eta0 = float(sc_eta)
    if not math.isfinite(eta0):
        raise AccuracyA33Error("sc_eta must be finite")

    damping = float(sc_damping)
    if damping < 0.0 or damping > 1.0 or not math.isfinite(damping):
        raise AccuracyA33Error("sc_damping must be in [0,1] and finite")

    clip = float(sc_clip)
    if clip <= 0.0 or not math.isfinite(clip):
        raise AccuracyA33Error("sc_clip must be > 0 and finite")

    rho_floor_val = float(rho_floor)
    if rho_floor_val < 0.0 or not math.isfinite(rho_floor_val):
        raise AccuracyA33Error("rho_floor must be >= 0 and finite")

    max_bt = int(sc_max_backtracks)
    if max_bt < 0 or max_bt > 3:
        raise AccuracyA33Error("sc_max_backtracks must be in [0,3]")

    m0 = np.asarray(mvec, dtype=float).reshape(-1)
    if m0.size != n:
        raise AccuracyA33Error("mvec must have shape (n,)")
    ...
    if not (math.isfinite(alpha) and math.isfinite(beta) and math.isfinite(gamma)):
        raise AccuracyA33Error("coef_alpha/beta/gamma must be finite")

    def _state_for_v(v_vec: np.ndarray) -> tuple[np.ndarray, float, np.ndarray, np.ndarray, float, float, float, float, float]:
        H_phase = lap_p + np.diag(np.asarray(v_vec, dtype=float))
        rho_raw_c, trace_heat, lambda_min, lambda_max, rho_trace_norm, rho_imag_max = (
            _heat_kernel_diagonal_shifted_trace_normalized_hermitian(H_phase, tau=float(heat_tau))
        )
        rho_raw = np.asarray(rho_raw_c, dtype=np.complex128).reshape(-1)
        if rho_raw.size != n:
            raise AccuracyA33Error("invalid rho shape")

        rho_real, rho_sum, rho_imag_max2, rho_renorm_applied, rho_renorm_delta = _rho_realness_and_normalize(
            rho_raw, rho_floor=float(rho_floor_val)
        )
        rho_imag_max = float(max(float(rho_imag_max), float(rho_imag_max2)))

        if rho_floor_val > 0.0:
            floor_mask = np.real(rho_raw) < rho_floor_val
        ...
        if not math.isfinite(E):
            raise AccuracyA33Error("non-finite E in SCF")

        return (
            np.asarray(rho_real, dtype=float),
            float(floor_rate),
    ...
    v = v0_vec.copy()
    rho, floor_rate, phi, curvature, E, rho_imag_max, rho_sum, rho_renorm_applied, rho_renorm_delta = _state_for_v(v)
    ...
    for t in range(iters):
        ...
        for bt in range(max_bt + 1):
            ...
            rho_c, floor_rate_c, phi_c, curvature_c, E_c, rho_imag_c, rho_sum_c, rho_renorm_c, rho_renorm_delta_c = _state_for_v(
                np.asarray(v_cand, dtype=float)
            )
            ...
            if E_c <= E_prev:
                ...
                E = float(E_c)
                rho_imag_max = float(rho_imag_c)
                rho_sum = float(rho_sum_c)
                rho_renorm_applied = bool(rho_renorm_c)
    ...
    return (
        ...
        bool(rho_renorm_applied),
        float(np.sum(rho)),
        float(-np.sum(rho * np.log(rho + 1e-300))),
        list(trace),
    )
...
def _phase_laplacian_for_row(*, weights: np.ndarray, cycles_heavy: Sequence[Sequence[int]], flux_phi: float) -> np.ndarray:
...
def _get_row_static(
    ...
    *,
    ...
    w_adj = _weighted_adjacency_from_bonds(
        ...
        aromatic_mult=float(cfg.edge_aromatic_mult),
...
def _predict_one(
    ...
    *,
    ...
    (
        ...
        rho_entropy,
        trace,
    ) = _solve_scf_full_functional_v1_phase_rho_pivot(
        laplacian_base=lap_base,
        laplacian_phase=lap_phase,
        v0=v0,
        heat_tau=float(cfg.heat_tau),
        phi_eps=float(cfg.phi_eps),
        rho_floor=float(cfg.rho_floor),
    ...
    pred_record: dict[str, object] = {
        ...
        "pred_raw": float(E),
        "phi_selected": float(normalize_flux_phi(float(flux_phi))),
        "rho_mode": "phase",
    }
...
def _select_phi_nested_train_only(
    *,
    rows: list[_Row],
    atoms_db: AtomsDbV1,
    cfg: _A33Config,
    ...
    for phi in PHI_CANDIDATES:
        ...
        agg = _aggregate_loocv_metrics(gm)
...
def run_a3_3(
    *,
    ...
    if len(group_ids) < 2:
        raise AccuracyA33Error("need at least 2 groups for LOOCV")
    ...
    for fold_id, test_gid in enumerate(fold_order, start=1):
        ...
        for r in test_rows:
            pred_rec, diag_rec = _predict_one(r, atoms_db=atoms_db, cfg=cfg, flux_phi=float(phi_sel))
        ...
        if gm_test is None:
            raise AccuracyA33Error("missing group metrics for test group")
        fold_rows.append(
            {
                "fold_id": int(fold_id),
                "test_group_id": str(test_gid),
                "group_id": str(test_gid),
                "phi_selected": float(phi_sel),
    ...
    rho_imag_max_max = float(max(float(r.get("rho_imag_max") or 0.0) for r in all_diag_records) if all_diag_records else 0.0)

    out_dir.mkdir(parents=True, exist_ok=True)

    predictions_path = out_dir / "predictions.csv"
    with predictions_path.open("w", encoding="utf-8", newline="") as f:
        fieldnames = ["fold_id", "id", "group_id", "smiles", "truth_rel_kcalmol", "pred_raw", "pred_rel", "rho_mode", "phi_selected"]
        w = csv.DictWriter(f, fieldnames=fieldnames, lineterminator="\n")
        w.writeheader()
        for rec in sorted(all_pred_records, key=lambda rr: (int(rr.get("fold_id") or 0), str(rr.get("group_id")), str(rr.get("id")))):
    ...
    with search_results_path.open("w", encoding="utf-8", newline="") as f:
        ...
        w = csv.DictWriter(f, fieldnames=fieldnames, lineterminator="\n")
        w.writeheader()
        for rec in sorted(search_results, key=lambda rr: (int(rr.get("fold_id") or 0), float(rr.get("phi_candidate") or 0.0))):
            w.writerow({k: rec.get(k, "") for k in fieldnames})
    ...
    metrics_payload: dict[str, object] = {
        "schema_version": "accuracy_a1_isomers_a3_3.v1",
        "experiment_id": str(experiment_id),
        "dataset": {"rows_total": int(len(rows_sorted)), "groups_total": int(len(group_ids))},
    ...
    index_lines = [
        f"# {experiment_id} (Isomers) A3.3 phase→rho pivot",
        "",
        ...
        "",
        "Nested Phi selection:",
        "```json",
        json.dumps(best_cfg.get("selected_phi_by_outer_fold", {}), ensure_ascii=False, sort_keys=True, indent=2),
        "```",
        "",
    ]
    ...
    extra_files: list[tuple[Path, Path]] = [
        ...
        (Path("docs/specs/accuracy_a3_3_phase_rho_pivot.md"), repo_root / "docs/specs/accuracy_a3_3_phase_rho_pivot.md"),
    ]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="ACCURACY-A3.3 phase→rho pivot runner (opt-in).")
    p.add_argument("--experiment_id", type=str, default="ACCURACY-A3.3", help="Experiment identifier for outputs.")
