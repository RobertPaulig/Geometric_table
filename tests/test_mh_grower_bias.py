...
def _run_mh(coupling_delta_G: float, temperature_T: float, seed: int = 123):
    rng = np.random.default_rng(seed)
    params = GrowthParams(max_depth=4, max_atoms=20)
    thermo = ThermoConfig(
        grower_use_mh=True,
    ...
    return proposals, accepted, rejected, rate, mol


def test_mh_grower_bias_reduces_acceptance_rate_at_low_T():
    proposals_hi, accepted_hi, rejected_hi, rate_hi, _ = _run_mh(
        coupling_delta_G=1.0,
        temperature_T=1e9,
        seed=111,
    )
    proposals_lo, accepted_lo, rejected_lo, rate_lo, _ = _run_mh(
        coupling_delta_G=1.0,
        temperature_T=0.1,
        seed=111,
    )
