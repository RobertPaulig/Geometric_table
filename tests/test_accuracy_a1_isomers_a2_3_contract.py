...
def test_accuracy_a1_isomers_a2_3_rho_law_trace_normalized_contract(tmp_path: Path) -> None:
    ...
    diag = list(csv.DictReader((out_dir / "diagnostics.csv").read_text(encoding="utf-8").splitlines()))
    assert len(diag) == 35
    assert {
        "rho_floor",
        "sc_damping",
        ...
        "gauge_fix_phi_mean",
        "sc_E_trace",
        "sc_accepted_backtracks_mean",
        "sc_accepted_backtracks_max",
        ...
        "lambda_min",
        "lambda_max",
    }.issubset(set(diag[0].keys()))

    metrics = json.loads((out_dir / "metrics.json").read_text(encoding="utf-8"))
    assert metrics["schema_version"] == "accuracy_a1_isomers_a2_3.v1"
    ...
    assert "diagnostics_csv" in metrics["files"]
