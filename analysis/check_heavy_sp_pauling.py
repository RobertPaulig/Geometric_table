...
def main() -> None:
    ...
    df_idx = pd.read_csv(DATA_DIR / "element_indices_with_dblock.csv")
    ...
    ref = pd.read_csv(DATA_DIR / "pauling_heavy_sp_reference.csv")
    ref = ref[ref["symbol"].isin(HEAVY_SP)]

    df_heavy = pd.merge(df_idx, ref, on="symbol", how="inner")
    if df_heavy.empty:
        out_path = RESULTS_DIR / "heavy_sp_pauling_fit.txt"
