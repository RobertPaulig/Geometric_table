...
def _thermo_for_mode(mode: str) -> ThermoConfig:
    ...
    if mode == "R":
        return ThermoConfig(
            experiment_name="CHEM-VALIDATION-1D-R",
            grower_use_mh=False,
    ...
    if mode == "A":
        return ThermoConfig(
            experiment_name="CHEM-VALIDATION-1D-A",
            grower_use_mh=True,
    ...
    if mode == "B":
        return ThermoConfig(
            experiment_name="CHEM-VALIDATION-1D-B",
            grower_use_mh=True,
            coupling_delta_G=1.0,
            coupling_complexity=1.0,
    ...
    if mode == "C":
        return ThermoConfig(
            experiment_name="CHEM-VALIDATION-1D-C",
            grower_use_mh=True,
            coupling_delta_G=1.0,
            coupling_complexity=1.0,
            coupling_topo_3d=1.0,
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> Config:
    ap = argparse.ArgumentParser(description="CHEM-VALIDATION-1D: C8 octane skeleton (proposal vs equilibrium).")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    out_stub = "chem_validation_1d_octane"
    ...
    with timed("growth_total", acc):
        run_iter = progress_iter(
            range(total_runs),
            total=total_runs,
            desc="CHEM-VALIDATION-1D growth runs",
        ...
        for mode in cfg.modes:
            ...
            for seed in cfg.seeds:
                ...
                for _ in range(int(cfg.n_runs)):
                    next(it)
                    params = GrowthParams(stop_at_n_atoms=n_atoms, allowed_symbols=["C"], max_depth=4)
    ...
    def _edges_for_start(spec: str) -> List[Edge]:
        if spec == "path":
            return _make_path_edges(8)
        if spec == "max_branch":
            return _make_max_branch_edges(8)
        ...
        for mode in [m for m in cfg.modes if m != "R"]:
            ...
            for start_spec in cfg.eq_start_specs:
                ...
                for chain_idx in progress_iter(
                    ...
                    desc=f"CHEM-VALIDATION-1D EQ mode{mode} start={start_spec}",
                    enabled=bool(cfg.progress),
                ):
            ...
            p_eq: Dict[str, float] = {
                k: float(np.mean(np.asarray([p.get(k, 0.0) for _, p, _, _ in per_start], dtype=float))) for k in keys_all
            }
    ...
    with timed("io_total", acc):
        ...
        lines: List[str] = []
        lines.append("CHEM-VALIDATION-1D: C8 octane skeleton (proposal vs equilibrium)")
    ...
    elapsed_total = time.perf_counter() - t0_total
    end_ts = now_iso()
    print(f"[CHEM-VALIDATION-1D] wrote {out_csv}")
    print(f"[CHEM-VALIDATION-1D] wrote {out_txt}")
