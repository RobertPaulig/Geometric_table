...
def _parse_args(argv: Optional[Sequence[str]] = None) -> Config:
    ap = argparse.ArgumentParser(description="CHEM-VALIDATION-2: C10 (decane) equilibrium-first + self-consistency.")
    ...
    ap.add_argument("--burnin", type=int, default=5_000)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    p_pred, energies, aut_sizes, g_vals = p_pred_from_energy(
        topo_keys, n=10, backend=backend, lam=float(cfg.lam), temperature_T=float(cfg.temperature_T)
    )
    write_report_and_csv(
        out_stub="chem_validation_2_decane",
        cfg=eq_cfg,
        p_eq=p_eq,
        meta=meta,
        p_pred=p_pred,
