...
def _parse_args(argv: Optional[Sequence[str]] = None) -> Config:
    ap = argparse.ArgumentParser(
        description="CHEM-VALIDATION-4: C13 (tridecane) equilibrium-first (tree-only)."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    expected = expected_unique_alkane_tree_topologies(13)
    ...
    write_report_and_csv(
        out_stub="chem_validation_4_tridecane",
        cfg=cfg,
        p_eq=p_eq,
        meta=meta,
        p_pred=p_pred,
